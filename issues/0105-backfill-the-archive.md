...
- [ ] A sweep exists that finds documents holding no passages and writes their passages
- [ ] It follows the shape of the existing unverified-document sweep
- [ ] It runs down to zero: a second run over a swept corpus does no work and reports so
- [ ] A backfilled document is retrievable on the same terms as a newly uploaded one
- [ ] Re-running the sweep never duplicates passages for a document
- [ ] The sweep is not added to any workflow
