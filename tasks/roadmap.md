...
- **Closed 2026-09-07** ([evidence](evidence/4.3.md)). FieldPromotion + stamp trigger + TenancyEvent
  `amended`. Direct stamp **`23001`**. Unmapped field cannot promote. R9 red first on a planted
  `extracted_field` in the isolation join. **427 code + 41 hooks**, policy 44. Staging sweep of
  week-3 unverified rows did not run (no serving revision) — carried to 4.4.
- **Deps:** 4.2 · **Size:** M
...
>
> **Amended 10 Sep 2026 by 5.1b**, which is Move 1 of the re-plan the director stopped the build
> for. Identity Platform, the password form, the TOTP form and the whole invite flow are gone; the
> credential is Google's and the session is still ours. 5.1 is not reopened — an amendment carries a
> letter, and both entries stay readable side by side, which is the record of the fact that this
> mechanism was built twice.

### Slice 5.1b — The credential is Google's, and the TOTP machinery goes
**Move 1 of the 10 Sep re-plan.** The director's own worked example was the admin login, and the
instinct — *more machinery than the job needs* — was right. The proposed replacement, a hand-rolled
email + password, is *more* code and is exactly [from-v3.md](../docs/from-v3.md)'s gap 1. So the
simplification runs the other way: **delegate the whole credential and delete our screens.** One
link → Google → our session, as a server-side OIDC authorization-code flow with a single redirect
and still no client JavaScript. Identity Platform is dropped entirely rather than kept as a
directory in front of Google; `google-auth-library` is already a dependency, so nothing new arrives.
**The invite becomes a row** — an admin adds `email` + `role`, that row *is* the authorisation, and
first sign-in fills `idp_local_id`. `staff_invite` is dropped.
- **What is lost, said plainly: the assertable second factor.** 5.1 refused any ID token without
  `firebase.sign_in_second_factor`; whatever Google enforces, we cannot assert it. What replaces it
  is narrower on the other axis — **only an email that already has a `staff_account` row may sign in
  at all**, where before it was anyone Identity Platform knew. That allowlist is what makes this
  safe without Workspace, and it **retires open question 12 as a blocker**. Written down as
  [ADR-0005](../docs/decisions/ADR-0005-the-credential-is-google-s.md).
- **Done when:** an operator added by `npm run staff:add` signs in through Google and holds a
  session; an unknown email, an unverified email, a role-less account and a disabled account are
  each refused with `not_allowed` and nothing more; a replayed or absent `state` is refused.
- **Verify:** every refusal red first; `npm run migrate` leaves **26 tables**; the sign-in clicked on
  `:3000` against the real client and not the fake — 5.1 found two defects a fake could not have.
- **One manual step, the director's:** Google exposes no API for creating an OAuth 2.0 Web client, so
  a human creates it and the consent screen once per project. `bootstrap.sh` documents it and does
  not pretend to do it.
- **Carried in from 5.0-cut:** `0022_` also drops `outbox` and `idempotency_keys` where they still
  stand — local and staging — because the ledger is by filename with no checksum and the squash
  reached neither. The same fact corrects the re-plan's instruction to squash `staff_invite` out of
  `0021_`: staging ran `0021_` at the 5.1 deploy, so a forward migration is the only thing that
  reaches it.
- **Deps:** 5.1 · **Size:** M
...
- **This is the slice that may lift the never-a-name rule.** Every screen shows a state and a count
  ...
  session arrived.
- **Carried in from 5.1:** the CSRF token's scope is **every** write route, the six staff POSTs
  included — 5.1 built no half of a token on purpose, and `SameSite=Lax` on the session cookie is
  what stands in for one until this slice. The guard is `requireStaff` from
  `src/staff/contract.ts`, called once per route and never re-implemented; the four staff screens
...
## Displaced, then archived · slices 7.1–9.5 — **14 Sep 2026**

> **Moved out of this file on 14 Sep 2026 to [archive/displaced-slices.md](archive/displaced-slices.md),
> and not deleted.** Three sections, seventeen slices, none of them built: the policy layer and the
> responsibility matrix (7.1–7.6), the ticket console walked by hand (8.1–8.6), and the live
> isolation attack that carried **M2** (9.1–9.5).
>
> **They were displaced on 13 Sep and archived on 14 Sep for different reasons.** The displacement
> was a sequencing call. The archiving is not: week 6's demo found four defects that eleven green
> slices and a full CI gate had not, and the director's conclusion is that the flow descriptions
> these slices were decomposed from — written in week 1 — no longer describe the system he wants.
> Building them correctly against a specification nobody still believes is the expensive mistake,
> and it is cheaper to make it now than in week 12.
>
> **The slice sequence continues from 7.1**, which is a different 7.1: the first slice of a
> UI-driven track that starts at the document intake. The numbers in the archive keep their old
> meanings inside that file and are not reused as live work.
>
> **The calendar and the month tables below are untouched**, as they always are. What the weeks
> meant is still what they meant; which slices deliver them is what changed.
>
> **Still owed, and archived with nothing:** redaction at the provider boundary, audit on every
> scoped read, the IAM pass (`run.admin` per service; the docs buckets' legacy delete-capable
> bindings), the emergency bypass, and the M2 checkpoint. Each is carried in
> [todo.md](todo.md) with an owner. The archive file names them at its head.
