...
Rev A2, 3 Sep 2026: the six D-decisions applied."""
...
lines = [
    ...
    ("sub", "Rev A2, 3 September 2026. Your six decisions are applied. Nothing here is saved to the "
            "repo or published. Minimal fields on purpose — the goal of this pass is to get the "
            "relationships right, then add fields on top of a shape that already holds."),
    ("gap", ""),
    ("h2", "Rev A2 — what your six answers changed"),
    ("n1", "D1  Space is in. No change to the draft; the shape below is now the agreed one."),
    ("n1", "D2  Project added above Building, and OPTIONAL. Building.project_code is replaced by a "
           "nullable Building.project_id. A standalone building simply leaves it null. Nothing "
           "downstream depends on Project yet — it exists so that Shoham can grow into it."),
    ("n1", "D3  Parking bays and storage rooms get Space rows. space_kind gains PARKING and STORAGE "
           "so the responsibility rule can see them for what they are."),
    ("n1", "D4  Guarantor locked. is_service_contact is no longer a default for GUARANTOR — it is "
           "false and cannot be set true. This is a constraint, not a preference."),
    ("n1", "D5  Obligation types become an admin-managed table, not a fixed list in code. The five "
           "you have are the seed rows. ObligationType is a new entity so that adding ועד בית "
           "variants or sub-metered water later is a screen, not a release."),
    ("n1", "D6  Condition stays a plain enum on Unit, separate from derived occupancy. No change."),
    ("gap", ""),
    ("n1", "Numbering note: the two new relationships are appended as R15 and R16 rather than "
           "inserted, so that R1–R14 keep the numbers you have already read — and the numbers in "
           "the Hebrew file. Entity numbers (E1–E14) did shift, since nothing cross-references them."),
    ("gap", ""),
    ...
    ("p",  "DECISIONS FOR YOU — the six forks, now settled, with what each one changed. Kept as a "
           "record rather than deleted, so nobody relitigates them in three months."),
]
...
f("Project", "project_code", "text", "yes", "",
  ...
  "Was Building.project_code in Rev A. One fact, one place.")
...
# ---------------------------------------------------------------- DECISIONS
dec = [
    ("D1", "Is Space worth the extra table?",
     "Add Space as the single 'where', with Unit as its leasable extension.",
     "Skip Space. Assets carry both a nullable unit_id and a nullable common_area_id.",
     "SETTLED — Space is in",
     "Without Space, every asset query branches on which location column is filled, and common areas "
     "quietly grow into a parallel system with their own screens. One table now removes that fork "
     "from every query we will ever write. CHANGED: nothing — the draft already assumed it. This is "
     "now the agreed shape, not a proposal."),
    ("D2", "Is a 'building' the right container for Shoham's 72 units?",
     "One Building row = one entrance core with one address.",
     "Add a Site/Project level above Building, if the 72 units span several cores under one tender.",
     "SETTLED — Project added, optional",
     "CHANGED: new entity Project (E1); Building.project_code replaced by a nullable "
     "Building.project_id; PROJECT added to DocumentLink.entity_type; new relationship R15. "
     "Optional means nullable and unused — nothing breaks if we never populate it. ONE EDGE LEFT "
     "OPEN, deliberately: Space.building_id is still mandatory, so a garden shared between three "
     "cores hangs off one of them. If Shoham is multi-core, that is the question to reopen, and "
     "having Project already there makes it cheap."),
    ("D3", "Do parking bays and storage rooms get their own Space rows?",
     "Space rows only where the bay is separately numbered and assignable.",
     "Plain attributes on Unit ('parking: P-14').",
     "SETTLED — Space rows",
     "CHANGED: space_kind gains PARKING and STORAGE; R4 extended so an assigned bay follows its "
     "unit and an unassigned one is treated as COMMON; a new panel on the unit screen and a new "
     "question Q7 on the test. A bay is now a place that can hold a gate motor, take a service "
     "call, and be reassigned without losing its history."),
    ("D4", "Can a guarantor (ערב) ever get service information?",
     "No. is_service_contact defaults to FALSE for GUARANTOR, and the agent will not engage them.",
     "Treat everyone named on a lease as a tenant.",
     "SETTLED — LOCKED, not defaulted",
     "CHANGED, and made stronger than drafted: it is no longer a default an admin could flip. "
     "is_service_contact is forced false whenever role = GUARANTOR — a database constraint, not a "
     "form setting. There is no switch to find and no import that can override it. The settings "
     "sheet records it as locked so nobody goes looking."),
    ("D5", "Is the obligation list complete?",
     "ARNONA · CONTENTS_INSURANCE · BANK_GUARANTEE · UTILITY_ACCOUNT · HOUSE_COMMITTEE.",
     "Something is missing — sub-metered water, gas, ועד בית handled differently per project.",
     "SETTLED — seed these five, admin extends",
     "CHANGED: obligation types are no longer an enum in code. New entity ObligationType (E10), "
     "Obligation.obligation_type becomes obligation_type_id, new relationship R16, new settings "
     "screen row. The five become seed rows. Two rules come with it: a type is deactivated and "
     "never deleted, and responsible_party is copied onto the obligation at creation so editing "
     "the catalogue never rewrites history."),
    ("D6", "Should condition (RENOVATION, WITHHELD) live on Unit?",
     "Yes — one small enum, separate from occupancy which stays derived.",
     "Model it as a kind of tenancy, so a unit is always 'held' by something.",
     "SETTLED — keep it simple",
     "CHANGED: nothing but the wording, which now says settled rather than proposed. The elegant "
     "version would have made every occupancy query handle a fake tenancy. Plain beats clever here."),
]
ws = sheet("DECISIONS FOR YOU",
           ["#", "The fork", "Option A — what I drafted", "Option B — the alternative",
            "Decided, 3 Sep 2026", "What it changed"],
           [5, 34, 42, 40, 26, 66],
           [tuple(x) for x in dec])
for row in range(2, 2 + len(dec)):
    ws.cell(row=row, column=5).font = Font(size=10, bold=True, color=GOOD)
